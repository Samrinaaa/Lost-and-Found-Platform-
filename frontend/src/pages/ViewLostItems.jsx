...
const ViewLostItems = () => {
  ...
  const fetchLostItems = async (page = 1) => {
    try {
      const res = await API.get(`/lost?search=${search}&page=${page}&limit=${ITEMS_PER_PAGE}`);
      ...
      <div style={styles.content}>
        ...
        {selectedItem && (
          <div style={styles.matchPanel}>
            <div style={styles.matchPanelHeader}>
              <div>
                <h3 style={styles.matchPanelTitle}>🔗 Potential matches for "{selectedItem.itemName}"</h3>
                <p style={styles.matchPanelSub}>Found items ranked by similarity in category, name, description and location.</p>
              </div>
              <button onClick={() => { setSelectedItem(null); setMatches([]); setMatchError(""); }} style={styles.closePanelBtn}>✕</button>
            </div>

            {matchLoading && (
              <div style={styles.matchLoading}><div style={styles.spinner} />Searching for matches…</div>
            )}
                  ...
                  <div key={match._id} style={styles.matchCard}>
                    ...
                    <div style={styles.matchCardBody}>
                      ...
                      <div style={styles.matchCardMeta}>
                        ...
                        {match.dateFound && <span>📅 {new Date(match.dateFound).toLocaleDateString()}</span>}
                      </div>
        ...
        {lostItems.length === 0 ? (
          <div style={styles.emptyState}>
            ...
            <h3 style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 20, color: "#2c3e3a", marginBottom: 8 }}>No lost items found</h3>
            <p style={{ color: "#5a6e6a", fontSize: 14 }}>{search ? "Try a different search term." : "No lost items have been reported yet."}</p>
          </div>
        ) : (
          <>
