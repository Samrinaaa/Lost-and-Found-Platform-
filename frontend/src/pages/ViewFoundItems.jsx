...
const ViewFoundItems = () => {
  ...
  const [message, setMessage] = useState("");
  const [search, setSearch] = useState("");
>>>>>>> 66cdc5e (Fix View all links on home page, improve image display)
  ...
  useEffect(() => { fetchFoundItems(); }, [search]);
      ...
      <div style={styles.content}>
        ...
        <div style={styles.header}>
          ...
          {currentUser ? (
            ...
            <Link to="/login">
              <button style={styles.backBtn}>Login to Report</button>
            </Link>
          )}
        </div>
        ...
        <div style={styles.toolbar}>
          <div>
            ...
            <p style={styles.subtitle}>{foundItems.length} report{foundItems.length !== 1 ? "s" : ""} found</p>
          </div>
        ...
        {selectedItem && (
          <div style={styles.matchPanel}>
            <div style={styles.matchPanelHeader}>
              <div>
                <h3 style={styles.matchPanelTitle}>
                  🔗 Potential lost item matches for "{selectedItem.itemName}"
                </h3>
                <p style={styles.matchPanelSub}>
                  Lost item reports ranked by similarity in category, name, description and location.
                </p>
              </div>
              <button
                onClick={() => { setSelectedItem(null); setMatches([]); setMatchError(""); }}
            ...
            {matchError && !matchLoading && (
              <div style={styles.matchEmpty}>{matchError}</div>
            )}
                ...
                {matches.map((match) => (
                  <div key={match._id} style={styles.matchCard}>
                    {match.imageUrl ? (
                      ...
                      <div style={styles.matchImgPlaceholder}>
                    ...
                    <div style={styles.matchCardBody}>
                      ...
                      <div style={styles.scoreBar}>
                        <div style={{
                          ...
                          background: match.matchScore >= 70
                            ? "linear-gradient(90deg, #2d6a64, #3d8a82)"
                            : match.matchScore >= 50
                      ...
                      {match.description && (
                        <p style={styles.matchCardDesc}>{match.description}</p>
                      )}

                      <div style={styles.matchCardMeta}>
                        {match.locationLost && <span>📍 {match.locationLost}</span>}
                        {match.dateLost && (
                          <span>📅 {new Date(match.dateLost).toLocaleDateString()}</span>
                        )}
                      ...
                      {currentUser && (
                        <Link to="/claim">
                          <button style={styles.claimBtn}>Submit Claim →</button>
                        </Link>
                      )}
                    </div>
                  </div>
        ...
        {foundItems.length === 0 ? (
          <div style={styles.emptyState}>
            ...
            <p style={{ color: "#5a6e6a", fontSize: 14 }}>
              {search ? "Try a different search term." : "No found items have been reported yet."}
            </p>
          </div>
        ) : (
            ...
            {foundItems.map((item) => {
              ...
              return (
                <div
                  ...
                  style={{
                    ...styles.card,
                    border: isSelected ? "2px solid #2d6a64" : "1px solid rgba(45,106,100,0.08)",
                  }}
                ...
                >
                  ...
                  <div style={styles.cardBody}>
                    <div style={{ display: "flex", gap: 8, marginBottom: 10, flexWrap: "wrap" }}>
                      ...
                      <span style={{ ...styles.statusBadge, ...(statusColors[item.status] || statusColors.open) }}>
                        {item.status}
                      </span>
                    </div>
                      ...
                      <button
                        ...
                        style={{
                          ...
                          background: isSelected
                            ? "linear-gradient(135deg, #2d6a64, #245854)"
                            : "transparent",
                          color: isSelected ? "#fff" : "#2d6a64",
                        }}
                      >
              ...
              >
                ...
                <div style={styles.cardBody}>
                  <div style={{ display: "flex", gap: 8, marginBottom: 10, flexWrap: "wrap" }}>
                    ...
                    <span style={{ ...styles.statusBadge, ...(statusColors[item.status] || statusColors.found) }}>
                      {item.status}
                    </span>
                  </div>
